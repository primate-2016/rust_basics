...
fn main() {
    ...
    // create a shadown of original guess var, useful to change type
    // parse method parses a string into a number - here a u32 (32-bit int)
    // if parse returns an error then print out the message...
    // we didn't set secret_number to a number type but Rust will infer it
    // based on compare below to u32 type for guess shadow
